...
namespace KingdomHeartsCustomMusic.utils
{
    ...
    internal static class ManagedSingleEncoder
    {
        ...
        public static void Encode(string inputScdTemplate, string inputWav, int quality, bool fullLoop, string encoderDir, Action<int>? progress = null)
        {
            ...
            Logger.Log($"Tools resolved | oggenc='{ogg}', adpcmencode3='{adpcm}', toolsDir='{ResolveToolsPath(encoderDir)}'");
            if (!File.Exists(ogg)) Logger.Log($"WARNING: oggenc.exe not found at {ogg}");
            if (!File.Exists(adpcm)) Logger.Log($"WARNING: adpcmencode3.exe not found at {adpcm}");

            // Strictly follow SingleEncoder_Program algorithm for a single SCD template with one stream.
            // Load original.scd template
            ...
            Logger.Log($"Template parsed | tables_offset=0x{tables_offset:X}, headers_entries={headers_entries}, headers_offset=0x{headers_offset:X}, initial_file_size={file_size}");
            ...
            Logger.Log($"Detected codec: 0x{codec:X}");
            ...
            for (int i = 0; i < headers_entries; i++)
            {
                ...
                Logger.Log($"Entry[{i}] begin=0x{entry_begin:X}, end=0x{entry_end:X}, size={entry_size}");
                ...
                {
                    Logger.Log($"Entry[{i}] is dummy; copying as-is");
                    newEntry = entry; // dummy entry
                }

                SCDs.Add(newEntry);
                file_size += newEntry.Length;
                entry_offsets[i + 1] = file_size;
                Logger.Log($"Entry[{i}] newSize={newEntry.Length}, cumulativeFileSize={file_size}");
            }
        ...
        private static void WavtoOGG(string inputWAV, int LoopStart_Sample, int Total_Samples, int Quality, string oggEncPath)
        {
            ...
            Logger.Log($"Run oggenc | exe='{p.StartInfo.FileName}', args='{p.StartInfo.Arguments}', cwd='{p.StartInfo.WorkingDirectory}'");
            p.Start(); p.BeginOutputReadLine(); p.BeginErrorReadLine(); p.WaitForExit();
            Logger.Log($"oggenc exit {p.ExitCode}\nSTDOUT:\n{sbOut}\nSTDERR:\n{sbErr}");
            if (p.ExitCode != 0)
            {
        ...
        {
            Logger.Log($"OGGtoSCD | ogg='{oggPath}', exists={File.Exists(oggPath)}");
            ...
            Logger.Log($"OGGtoSCD | pages found={page_offsets.Count}");
            ...
            for (int i = 0; i < page_offsets.Count; i++)
            {
                ...
                Logger.Log($"OGGtoSCD | page[{i}] @0x{idx:X}, granule={granule}");
                if (granule != 0)
                {
            ...
            Logger.Log($"OGGtoSCD | headerSize={vorbis_header_size}");
            ...
            Logger.Log($"OGGtoSCD | newEntrySize={newEntry.Length}");
            return newEntry;
        }
        ...
        private static void WavtoMSADPCM(string inputWAV, string adpcmEncPath, string encoderDir)
        {
            ...
            p.Start(); p.BeginOutputReadLine(); p.BeginErrorReadLine(); p.WaitForExit();
            Logger.Log($"adpcmencode3 exit {p.ExitCode}\nSTDOUT:\n{sbOut}\nSTDERR:\n{sbErr}");
